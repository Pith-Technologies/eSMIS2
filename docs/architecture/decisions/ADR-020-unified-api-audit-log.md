...
The `tpl_api_v2` module (planned) will have two separate audit mechanisms:

1. **tpl.consent.access.log** - Logs read operations (read, search, export) with consent context
2. **tpl.audit.log** (from `tpl_audit`, planned) - Logs data modifications (create, write, unlink) via decorator
...
Create a **unified tpl.api.audit.log** model (planned) that:
...
3. **Links consent** (via `tpl_consent`, planned) when applicable
4. **Links to tpl.audit.log** (via `tpl_audit`, planned) for field-level change details on write operations
5. **Replaces tpl.consent.access.log** (deprecated, kept for backwards compatibility)
...
│   └── audit_log_id → tpl.audit.log (links to field changes)
├── Result
...
3. **Link to tpl.audit.log**: After create/update, find matching audit log entry by model+res_id+timestamp
...
- Links to detailed field-level changes via tpl.audit.log
...
- Migration needed for existing tpl.consent.access.log data
- Slight increase in storage requirements

### Migration Path
1. Keep tpl.consent.access.log working (deprecated)
2. New operations use tpl.api.audit.log
