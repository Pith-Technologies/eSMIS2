...
class TestStockMove(TransactionCase):
    """Tests for esmis.stock.move model."""

    def setUp(self):
        super().setUp()
        # Use tracking_disable to suppress mail side effects
        self.warehouse = self.env["esmis.warehouse"].with_context(
...
|-------------|-----------------|
| Core (`esmis_vocabulary`, `esmis_project`) | 85% |
| API modules | 90% |
| Utility/helper | 80% |
| UI-only | 60% |
...
def test_officer_can_create_stock_move(self):
    ...
    officer = self.env.ref("esmis_vocabulary_demo.demo_officer")
    move = self.env["esmis.stock.move"].with_user(officer).create({
        "warehouse_id": self.warehouse.id,
        "quantity": 50,
    })
...
def test_viewer_cannot_create_stock_move(self):
    ...
    viewer = self.env.ref("esmis_vocabulary_demo.demo_viewer")
    with self.assertRaises(AccessError):
        self.env["esmis.stock.move"].with_user(viewer).create({
            "warehouse_id": self.warehouse.id,
            "quantity": 50,
        })
```
...
def test_demo_stock_move_records_exist(self):
    ...
    moves = self.env["esmis.stock.move"].search([])
    self.assertTrue(
        moves,
        "Demo data should have created at least one esmis.stock.move record",
    )
```
...
approval = self.env["esmis.approval.review"].create({
    "definition_id": self.approval_definition.id,
    "record_ref": f"esmis.order,{self.order.id}",
    "group_id": self.approver_group.id,
})
approval.with_user(self.manager).action_approve()
```

When creating an `esmis.approval.definition`, always include `approval_group_id` — it is required even when `approval_type` defaults to `group`.

```python
cls.approval_definition = cls.env["esmis.approval.definition"].create({
    "name": "Test Approval",
    "model_id": model_id,
    "approval_type": "group",
...
**Related models missing ACLs**: If `esmis.project` has an ACL entry, every related model it references (`esmis.task`, `esmis.note`) also needs its own ACL entries. Missing entries cause `AccessError` when loading the form view.
