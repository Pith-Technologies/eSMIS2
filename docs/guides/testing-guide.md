...
./odoo-project test tpl_project

# Filter by test tags
./odoo-project test tpl_inventory --tags=post_install
```
...
class TestStockMove(TransactionCase):
    """Tests for tpl.stock.move model."""

    def setUp(self):
        super().setUp()
        # Use tracking_disable to suppress mail side effects
        self.warehouse = self.env["tpl.warehouse"].with_context(
...
|-------------|-----------------|
| Core (`tpl_vocabulary`, `tpl_project`) | 85% |
| API modules | 90% |
| Utility/helper | 80% |
| UI-only | 60% |
...
def test_officer_can_create_stock_move(self):
    ...
    officer = self.env.ref("tpl_vocabulary_demo.demo_officer")
    move = self.env["tpl.stock.move"].with_user(officer).create({
        "warehouse_id": self.warehouse.id,
        "quantity": 50,
    })
...
def test_viewer_cannot_create_stock_move(self):
    ...
    viewer = self.env.ref("tpl_vocabulary_demo.demo_viewer")
    with self.assertRaises(AccessError):
        self.env["tpl.stock.move"].with_user(viewer).create({
            "warehouse_id": self.warehouse.id,
            "quantity": 50,
        })
```
...
def test_demo_stock_move_records_exist(self):
    ...
    moves = self.env["tpl.stock.move"].search([])
    self.assertTrue(
        moves,
        "Demo data should have created at least one tpl.stock.move record",
    )
```
...
approval = self.env["tpl.approval.review"].create({
    "definition_id": self.approval_definition.id,
    "record_ref": f"tpl.order,{self.order.id}",
    "group_id": self.approver_group.id,
})
approval.with_user(self.manager).action_approve()
```

When creating an `tpl.approval.definition`, always include `approval_group_id` — it is required even when `approval_type` defaults to `group`.

```python
cls.approval_definition = cls.env["tpl.approval.definition"].create({
    "name": "Test Approval",
    "model_id": model_id,
    "approval_type": "group",
...
**Related models missing ACLs**: If `tpl.project` has an ACL entry, every related model it references (`tpl.task`, `tpl.note`) also needs its own ACL entries. Missing entries cause `AccessError` when loading the form view.
